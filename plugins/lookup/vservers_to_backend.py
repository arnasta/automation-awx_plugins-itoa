...
DOCUMENTATION = r"""
  ...
  options:
    ...
    adc_hostname:
      ...
      type: string
      ini:
        - section: vservers_to_backend
          key: adc_hostname
    vserver:
      description: vserver name in ADC
      required: true
      type: string
    vserver_type:
      description:
...
- hosts: localhost
  ...
  vars:
    ...
    ns_proxies: "{{ lookup('cencora.itoa.vservers_to_backend', input_url, username=username, password=password) }}"
  tasks:
    - debug:
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        ret = []
        for term in terms:
            display.v("vservers_to_backend lookup term: %s" % term)
