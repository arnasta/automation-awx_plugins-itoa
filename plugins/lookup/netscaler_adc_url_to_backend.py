...
DOCUMENTATION = r"""
  name: netscaler_adc_url_to_backend
  ...
  options:
    ...
    adm_hostname:
      ...
      type: string
      ini:
        - section: netscaler_adc_url_to_backend
          key: adm_hostname
    username:
      description:
...
- hosts: localhost
  ...
  vars:
    ...
    backend_servers: "{{ lookup('cencora.itoa.netscaler_adc_url_to_backend', input_url, username=username, password=password) }}"
  tasks:
    - debug:
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        ret = []
        for term in terms:
            display.v("netscaler_adc_url_to_backend lookup term: %s" % term)
