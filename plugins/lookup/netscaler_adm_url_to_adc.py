...
DOCUMENTATION = r"""
  ...
  options:
    ...
    adm_hostname:
      ...
      type: string
      ini:
        - section: netscaler_adm_url_to_adc
          key: adm_hostname
    username:
      description:
...
- hosts: localhost
  ...
  vars:
    ...
    ns_proxies: "{{ lookup('cencora.itoa.netscaler_adm_url_to_adc', input_url, username=username, password=password) }}"
  tasks:
    - debug:
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        ret = []
        for term in terms:
            display.v("netscaler_adm_url_to_adc lookup term: %s" % term)
